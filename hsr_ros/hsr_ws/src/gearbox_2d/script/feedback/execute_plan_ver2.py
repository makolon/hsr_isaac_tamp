...
class ExecutePlan(object):
        ...
        if move_mode == 'outward':
            ...
            for i in range(1, len(diff_ee_traj)):
                ...
                if ee_mode == 'horizontal':
                    ...
                    goal_pose.position.y = diff_ee_traj[i][1] - 0.12
                    ...
                    goal_pose.position.x = diff_ee_traj[i][0]
                    goal_pose.position.y = diff_ee_traj[i][1] - 0.12
                    goal_pose.position.z = diff_ee_traj[i][2] - 0.2
                    goal_pose.orientation.x = 0.0
                    goal_pose.orientation.y = 0.707106781
                    goal_pose.orientation.z = 0.707106781
                    goal_pose.orientation.w = 0.0
    ...
    def set_planning_request(self, goal_pose):
        planning_request = PlanWithHandGoalsRequest()
        planning_request.origin_to_basejoint = Pose()
        planning_request.initial_joint_state = JointState() # sensor_msgs.msg
        planning_request.use_joints = ['']
        planning_request.ref_frame_id = 'map'
        planning_request.origin_to_hand_goals = [goal_pose]
        planning_request.timeout = rospy.Duration(3)
        return planning_request

    def generate_trajectory(self, planning_request):
        rospy.wait_for_service('plan_with_hand_goals')
        try:
            planner = rospy.ServiceProxy('plan_with_hand_goals', PlanWithHandGoals)
            arm_traj, base_traj, _, _, _, _ = planner(planning_request)
            return arm_traj, base_traj
