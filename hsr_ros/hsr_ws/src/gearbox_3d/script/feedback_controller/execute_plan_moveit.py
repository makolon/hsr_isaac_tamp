...
class ExecutePlan(object):
    ...
    def augment_plan(self, plan):
        # Replay_trajectory
        return self.tamp_planner.execute(plan, moveit=True)
    ...
    def check_move_status(self, target_ee_pose):
        ...
        print('move distance:', pos_dist)

        move_success = torch.where(
            pos_dist < torch.tensor([0.05]),
            torch.ones((1,)),
            torch.zeros((1,))
        )
        return move_success
    ...
    def check_place_status(self, target_ee_pose):
        target_ee_pos, target_ee_rot = target_ee_pose
        curr_ee_pos, curr_ee_rot = self.mocap_interface.get_pose('end_effector')
    ...
    def check_insert_status(self, target_ee_pose):
        target_ee_pos, target_ee_rot = target_ee_pose
        curr_ee_pos, curr_ee_rot = self.mocap_interface.get_pose('end_effector')
    ...
    def calculate_command(self, target_pose):
        curr_pos, curr_rot = self.mocap_interface.get_pose('end_effector')
        target_pos, target_rot = target_pose
        diff_pos = np.array(target_pos) - np.array(curr_pos)
        diff_rot = np.array(target_rot) - np.array(curr_rot)
        kp = 1.0
        command_pos, command_rot = kp * diff_pos, kp * diff_rot
        command_pos += np.array(curr_pos)
        command_rot += np.array(curr_rot)
        return (command_pos, command_rot)
    ...
    def execute(self):
        ...
        for i, (action_name, args) in enumerate(plan):
            if action_name == 'move_base':
                finish = False
                move_traj = move_metadata['target_robot_pose'][str(move_cnt)]['move']
                for target_pose in move_traj:
                    target_pose = self.calculate_command(target_pose)
                    goal_pose = self.set_goal_pose(target_pose)
                ...
                finish = False
                pick_traj = pick_metadata['target_robot_pose'][str(pick_cnt)]['pick']
                ...
                return_traj = pick_metadata['target_robot_pose'][str(pick_cnt)]['return']
                for target_pose in return_traj:
                    target_pose = self.calculate_command(target_pose)
                    goal_pose = self.set_goal_pose(target_pose)
                    (traj, fraction) = self.whole_body_moveit.compute_cartesian_path(goal_pose, 0.01, 0.0, False)
                    self.whole_body_moveit.execute(traj, wait=True)
                ...
                finish = False
                place_traj = place_metadata['target_robot_pose'][str(place_cnt)]['place']
                ...
                finish = False
                insert_traj = insert_metadata['target_robot_pose'][str(insert_cnt)]['insert']
                for target_pose in insert_traj:
                    target_pose = self.calculate_command(target_pose)
                    goal_pose = self.set_goal_pose(target_pose)
                    (traj, fraction) = self.whole_body_moveit.compute_cartesian_path(goal_pose, 0.01, 0.0, False)
                    self.whole_body_moveit.execute(traj, wait=True)
                ...
                depart_traj = insert_metadata['target_robot_pose'][str(insert_cnt)]['depart']
                for target_pose in depart_traj:
                    target_pose = self.calculate_command(target_pose)
                    goal_pose = self.set_goal_pose(target_pose)
                    (traj, fraction) = self.whole_body_moveit.compute_cartesian_path(goal_pose, 0.01, 0.0, False)
                    self.whole_body_moveit.execute(traj, wait=True)
                ...
                return_traj = insert_metadata['target_robot_pose'][str(insert_cnt)]['return']
                for target_pose in return_traj:
                    target_pose = self.calculate_command(target_pose)
                    goal_pose = self.set_goal_pose(target_pose)
                    (traj, fraction) = self.whole_body_moveit.compute_cartesian_path(goal_pose, 0.01, 0.0, False)
