...
def get_motion_gen(problem, custom_limits={}, collisions=True, teleport=False, motion_plan=False, resolution=0.05):
    robot = problem.robot
    ...
    def fn(bq1, bq2, fluents=[]):
        ...
        resolutions = resolution * np.ones(len(bq2.joints))
        if teleport:
            path = [bq1, bq2]
        else:
        ...
        if motion_plan:
            goal_conf = pose_from_base_values(bq2.values)
            raw_path = plan_base_motion(robot, goal_conf, BASE_LIMITS,
                                        obstacles=obstacles, resolutions=resolutions/2.)
            if raw_path is None:
                print('Failed motion plan!')
                return None
    ...
    for action_name, target_object_name, exp_command in commands:
        ...
        for command in exp_command:
            for j, _ in enumerate(command.apply(state, **kwargs)):
                ...
                if time_step is None:
                    wait_for_duration(1e-2)
                    wait_if_gui('Command, Step {}) Next?'.format(j))
                else:
                    wait_for_duration(time_step)
...
def create_dataset(problem_name, robot, objects, object_names, state, commands, time_step=None, pause=False, **kwargs):
    ...
    # Create dataset
    trajectory_path = os.path.join(os.environ['PYTHONPATH'].split(':')[1], 'hsr_tamp', 'experiments', 'gearbox_3d',
    ...
    metadata_path = os.path.join(os.environ['PYTHONPATH'].split(':')[1], 'hsr_tamp', 'experiments', 'gearbox_3d',
...
def replay_trajectory(robot, objects, object_names, state, commands, debug, **kwargs):
    movable_joints = get_base_arm_joints(robot, 'arm') # 8 joints
    ...
    # Prepare trajectory dict for all pose / positions
    move_metadata = {'init_robot_pose': [], 'goal_robot_pose': [], 'target_robot_pose': [],
                    'target_object_pose': []}
    ...
    for action_name, target_object_name, command in commands:
        # Add metadata (init_data)
        if action_name == 'move_base':
            move_metadata['init_robot_pose'].append(get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm'])))
        ...
        for c in command:
            ...
            if skill_clip_index[action_name] == command_num:
                if action_name == 'move_base':
                    if debug:
                        move_ee_pos, _ = get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm']))
                        create_marker(move_ee_pos)
                        print('move_ee_pos:', move_ee_pos)
                        move_joint_pos = get_joint_positions(robot, movable_joints)
                        print('move_joint_pos:', move_joint_pos)
                    move_metadata['target_robot_pose'].append(get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm'])))
                elif action_name == 'pick':
                    if debug:
                        pick_ee_pos, _ = get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm']))
                        create_marker(pick_ee_pos)
                        print('pick_ee_pos:', pick_ee_pos)
                        pick_joint_pos = get_joint_positions(robot, movable_joints)
                        print('pick_joint_pos:', pick_joint_pos)
                    pick_metadata['target_robot_pose'].append(get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm'])))
                elif action_name == 'place':
                    if debug:
                        place_ee_pos, _ = get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm']))
                        create_marker(place_ee_pos)
                        print('place_ee_pos:', place_ee_pos)
                        place_joint_pos = get_joint_positions(robot, movable_joints)
                        print('place_joint_pos:', place_joint_pos)
                    place_metadata['target_robot_pose'].append(get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm'])))
                elif action_name == 'insert':
                    if debug:
                        insert_ee_pos, _ = get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm']))
                        create_marker(insert_ee_pos)
                        print('insert_ee_pos:', insert_ee_pos)
                        insert_joint_pos = get_joint_positions(robot, movable_joints)
                        print('insert_joint_pos:', insert_joint_pos)
        ...
        # Add metadata (goal_data)
        if action_name == 'move_base':
            move_metadata['goal_robot_pose'].append(get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm'])))
        elif action_name == 'pick':
            ...
            for obj in objects:
                if object_names[obj] == target_object_name:
                    if debug:
                        pick_obj_pos, _ = get_pose(obj)
                        create_marker(pick_obj_pos, rgba=[0.0, 0.0, 1.0, 1.0])
                        print('pick_obj_pos:', pick_obj_pos)
                        input('wait_for_user')
            ...
            for obj in objects:
                if object_names[obj] == target_object_name:
                    if debug:
                        place_obj_pos, _ = get_pose(obj)
                        create_marker(place_obj_pos, rgba=[0.0, 0.0, 1.0, 1.0])
                        print('place_obj_pos:', place_obj_pos)
                        input('wait_for_user')
            ...
            for obj in objects:
                if object_names[obj] == target_object_name:
                    if debug:
                        insert_obj_pos, _ = get_pose(obj)
                        create_marker(insert_obj_pos, rgba=[0.0, 0.0, 1.0, 1.0])
                        print('insert_obj_pos:', insert_obj_pos)
                        input('wait_for_user')
                    insert_metadata['target_object_pose'].append(get_pose(obj))

    return move_metadata, pick_metadata, place_metadata, insert_metadata

def replay_trajectory_moveit(robot, objects, object_names, state, commands, debug, **kwargs):
    # Prepare trajectory dict for all pose / positions
    move_metadata = {'target_robot_pose': {'0': {'move': []}, '1': {'move': []}, '2': {'move': []},
                                '3': {'move': []}, '4': {'move': []}},
                     'target_object_pose': {'0': [], '1': [], '2': [], '3': [], '4': []}}
    pick_metadata = {'target_robot_pose': {'0': {'move': []}, '1': {'move': []}, '2': {'move': []},
                                '3': {'move': []}, '4': {'move': []}},
                     'target_object_pose': {'0': [], '1': [], '2': [], '3': [], '4': []}}
    place_metadata = {'target_robot_pose': {'0': {'move': []}, '1': {'move': []}, '2': {'move': []},
                                '3': {'move': []}, '4': {'move': []}},
                      'target_object_pose': {'0': [], '1': [], '2': [], '3': [], '4': []}}
    insert_metadata = {'target_robot_pose': {'0': {'move': []}, '1': {'move': []}, '2': {'move': []},
                                '3': {'move': []}, '4': {'move': []}},
                       'target_object_pose': {'0': [], '1': [], '2': [], '3': [], '4': []}}
    ...
    for action_name, target_object_name, command in commands:
        ...
        for c in command:
            for j, _ in enumerate(c.apply(state, **kwargs)):
                ...
                # Add metadata (goal_data)
                if action_name == 'move_base':
                    command_name = 'move' # TODO: temp
                    ee_pose = get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm']))
                    move_metadata['target_robot_pose'][str(move_cnt)][command_name].append(ee_pose)

                elif action_name == 'pick':
                    command_name = 'pick' # TODO: tmep
                    ee_pose = get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm']))
                    pick_metadata['target_robot_pose'][str(pick_cnt)][command_name].append(ee_pose)
                    for obj in objects:
                        if object_names[obj] == target_object_name:
                            obj_pose = get_pose(obj)
                            pick_metadata['target_object_pose'][str(pick_cnt)].append(obj_pose)

                elif action_name == 'place':
                    command_name = 'place' # TODO: temp
                    ee_pose = get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm']))
                    place_metadata['target_robot_pose'][str(place_cnt)][command_name].append(ee_pose)
                    for obj in objects:
                        if object_names[obj] == target_object_name:
                            obj_pose = get_pose(obj)
                            place_metadata['target_object_pose'][str(place_cnt)].append(obj_pose)

                elif action_name == 'insert':
                    command_name = 'insert' # TODO: temp
                    ee_pose = get_link_pose(robot, link_from_name(robot, HSR_TOOL_FRAMES['arm']))
                    insert_metadata['target_robot_pose'][str(insert_cnt)][command_name].append(ee_pose)
                    for obj in objects:
                        if object_names[obj] == target_object_name:
                            obj_pose = get_pose(obj)
                            insert_metadata['target_object_pose'][str(insert_cnt)].append(obj_pose)
...
def deterministic_pick(obj, pose):
    ...
    if obj == 3: # green gear
        pick_pose = ((pick_pose[0][0]-0.13, pick_pose[0][1], pick_pose[0][2]+0.01), pick_pose[1])
        approach_pose = ((pick_pose[0][0]-0.12, pick_pose[0][1], pick_pose[0][2]+0.05), pick_pose[1])
        ...
        pick_pose = ((pick_pose[0][0]-0.14, pick_pose[0][1], pick_pose[0][2]+0.01), pick_pose[1])
        approach_pose = ((pick_pose[0][0]-0.12, pick_pose[0][1], pick_pose[0][2]+0.05), pick_pose[1])
        ...
        pick_pose = ((pick_pose[0][0]-0.08, pick_pose[0][1], pick_pose[0][2]-0.02), pick_pose[1])
        approach_pose = ((pick_pose[0][0]-0.05, pick_pose[0][1], pick_pose[0][2]+0.05), pick_pose[1])
...
def deterministic_place(obj, pose):
    ...
    if obj == 3: # green gear
        place_pose = ((place_pose[0][0]-0.13, place_pose[0][1], place_pose[0][2]), place_pose[1])
        ...
        place_pose = ((place_pose[0][0]-0.14, place_pose[0][1], place_pose[0][2]), place_pose[1])
        approach_pose = ((place_pose[0][0], place_pose[0][1], place_pose[0][2]+0.02), place_pose[1])
        ...
        place_pose = ((place_pose[0][0]-0.08, place_pose[0][1], place_pose[0][2]), place_pose[1])
        approach_pose = ((place_pose[0][0], place_pose[0][1], place_pose[0][2]+0.02), place_pose[1])
...
def deterministic_insert(obj, pose):
    ...
    if obj == 3: # green gear
        insert_pose = ((insert_pose[0][0]-0.13, insert_pose[0][1], insert_pose[0][2]), insert_pose[1])
        ...
        insert_pose = ((insert_pose[0][0]-0.14, insert_pose[0][1], insert_pose[0][2]), insert_pose[1])
        ...
        insert_pose = ((insert_pose[0][0]-0.08, insert_pose[0][1], insert_pose[0][2]), insert_pose[1])
